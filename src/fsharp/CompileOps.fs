...
type TcConfigBuilder =
    ...
    member private tcConfigB.AddAssemblyByPath (m, path, lst: AssemblyReference list) = 
        if FileSystem.IsInvalidPathShim(path) then
            warning(Error(FSComp.SR.buildInvalidAssemblyName(path), m))
            lst
        elif not (lst |> List.exists (fun ar2 -> m=ar2.Range && path=ar2.Text)) then // NOTE: We keep same paths if range is different.
            let projectReference = tcConfigB.projectReferences |> List.tryPick (fun pr -> if pr.FileName = path then Some pr else None)
            lst ++ AssemblyReference(m, path, projectReference)
        else
            lst

    member private tcConfigB.RemoveAssemblyByPath (m, path, lst: AssemblyReference list) =
        lst |> List.filter (fun ar-> ar.Range <> m || ar.Text <> path)
    
    member tcConfigB.AddReferencedAssemblyByPath (m, path) =
        tcConfigB.referencedDLLs <- tcConfigB.AddAssemblyByPath(m, path, tcConfigB.referencedDLLs)
             
    member tcConfigB.RemoveReferencedAssemblyByPath (m, path) =
        tcConfigB.referencedDLLs <- tcConfigB.RemoveAssemblyByPath(m, path, tcConfigB.referencedDLLs)
    
    member tcConfigB.AddTranslatorAssemblyByPath (m, path) = 
        tcConfigB.translatorDLLs <- tcConfigB.AddAssemblyByPath(m, path, tcConfigB.translatorDLLs)
             
    member tcConfigB.RemoveTranslatorAssemblyByPath (m, path) =
        tcConfigB.translatorDLLs <- tcConfigB.RemoveAssemblyByPath(m, path, tcConfigB.translatorDLLs)
...
// "there is no really good notion of runtime directory on .NETCore"
    ...
    interface ICompilerConfig with
        member x.primaryAssembly = data.primaryAssembly
        member x.autoResolveOpenDirectivesToDlls = data.autoResolveOpenDirectivesToDlls
        member x.noFeedback = data.noFeedback
        member x.stackReserveSize = data.stackReserveSize   
...
let LoadTranslators (tcConfig:TcConfig) =
    ...
    |> Seq.collect (fun ar ->
        ...
        assembly.GetCustomAttributes(typeof<TranslatorAttribute>, false)
            |> Seq.map (fun attr ->
                let targetType = (attr :?> TranslatorAttribute).TargetType
                let translator = Activator.CreateInstance(targetType)
                translator :?> ITranslator))
    |> Seq.toList
           
// Get translators from translator instances.
let GetTranslators<'Node> (translators: ITranslator list) =
    translators
    |> List.choose (fun tr ->
        match tr with
        | :? ITranslator<ICompilerConfig, ErrorLogger, 'Node> as tr -> Some(tr)
        | _ -> None)

// Apply translators to AST.
let ApplyTranslators<'Node> (tcConfig: TcConfig, translators: ITranslator<ICompilerConfig, ErrorLogger, 'Node> list, errorLogger: ErrorLogger, input: 'Node) =
    translators |> List.fold (fun input tr -> tr.Translate (tcConfig :> ICompilerConfig) errorLogger input) input
